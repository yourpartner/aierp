      ...
      <el-row v-if="block.type==='grid'" :gutter="12" style="margin-bottom:8px;">
        <template v-for="(col, cidx) in block.cols" :key="cidx">
          <el-col :span="col.span || 6" v-if="isVisible(col)">
            <el-form-item :label="labelOf(col)">
                ...
                <template v-if="isArrayField(col)">
                  <div class="array-editor">
                    <div style="display:flex;gap:8px;align-items:center">
                      <el-button size="small" @click="addArrayRow(col.field)">{{ dynamicText.value.addRow }}</el-button>
                    ...
                    <el-table :data="getArray(col.field)" size="small" border style="margin-top:6px">
                      <el-table-column v-for="c in arrayColumns(col.field)" :key="c.field" :label="translateSchemaLabel(c.label, c.field, 'field')" :width="c.width">
                        <template #default="{ row }">
                          ...
                          <template v-else-if="(c.widget||'')==='select'">
                            <el-select v-model="row[c.field]" filterable clearable style="width:100%" v-bind="selectOnlyProps(c)">
                      ...
                      <el-table-column :label="dynamicText.value.actions" width="80">
                        <template #default="{ $index }">
                          <el-button size="small" type="danger" @click="removeArrayRow(col.field, $index)">{{ dynamicText.value.removeRow }}</el-button>
                        </template>
                      </el-table-column>
                    </el-table>
                  </div>
...
const props = defineProps<{ ui: any; model: any; schema?: any; arrayFields?: string[]; columnsOverride?: Record<string, any[]> }>()
...
function addArrayRow(path:string){
  ...
  const obj:any = {}
  for (const c of cols){ obj[c.field] = c.default ?? (c.inputType==='number'?0 : '') }
