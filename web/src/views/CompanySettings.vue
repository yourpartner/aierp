    ...
    <el-card>
      <template #header>
        <div class="page-header">
          <div class="page-header-title">公司设置</div>
      ...
      <div v-else style="color:#9ca3af">正在加载 schema...</div>
    </el-card>
  </div>
</template>
...
const model = reactive<any>({ taxExemptFrom: '', taxExemptTo: '', inputTaxAccountCode: '', outputTaxAccountCode: '' })
  ...
  try{
    ...
    const doc = {
      ...
      ui: { form: { layout: [ { type:'grid', cols:[
        { field:'companyName', label:'公司名称', span:12 },
        { field:'companyAddress', label:'公司地址', span:12 },
        { field:'companyRep', label:'代表者', span:6 },
        { field:'workdayDefaultStart', label:'上班(HH:mm)', span:6 },
        { field:'workdayDefaultEnd', label:'下班(HH:mm)', span:6 },
        { field:'lunchMinutes', label:'午休(分钟)', span:6, props:{ type:'number' } }
      ] } ] } },
...
async function load(){
  ...
  try{
    ...
    if (rows.length>0){
      ...
      // 默认值
      Object.assign(model, { workdayDefaultStart:'09:00', workdayDefaultEnd:'18:00', lunchMinutes:60, taxExemptFrom:'', taxExemptTo:'', inputTaxAccountCode:'', outputTaxAccountCode:'' })
      currentId.value = ''
      ensureSealDefaults()
      ensureTaxExemptDefaults()
      ensureTaxAccountDefaults()
...
function onPickSeal(e: Event){
  ...
  reader.onload = () => {
    ...
    ElMessage.success('印章已选取，保存后将加密存储')
  }
