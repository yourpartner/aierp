<template>
  <div class="page page-medium">
    <el-card>
      ...
      <template v-else>
        <DynamicForm v-if="!uiLoading && ui" :ui="ui" :model="form" @action="onAction" />
        <el-skeleton v-else :rows="6" animated />
      </template>
      <el-dialog v-model="showBank" :title="accountText.bankDialog" width="720px">
        <BankBranchPicker mode="bank" @select="onPickBank" @cancel="showBank = false" />
      </el-dialog>
      <el-dialog v-model="showBranch" :title="accountText.branchDialog" width="720px">
...
const tableLabels = section({ title:'', new:'', code:'', name:'', type:'', status:'' }, (msg) => msg.tables.partners)
...
const form = reactive<any>({ flags: { customer: false, vendor: false } })
...
async function save() {
  ...
  try {
    ...
    await api.post('/objects/businesspartner', body)
