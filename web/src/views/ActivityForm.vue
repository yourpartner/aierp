...
const tableLabels = section({ title: '', new: '', subject: '', type: '', dueDate: '', owner: '', status: '' }, (msg) => msg.tables.activities)
...
const form = reactive<any>({ type: 'task', status: 'open' })
...
onMounted(async () => {
  ...
  uiError.value = ''
  try {
    const r = await api.get('/schemas/activity', { params: { lang: lang.value } })
...
async function save() {
  ...
  try {
    ...
    await api.post('/objects/activity', body)
