<template>
  <div style="padding:16px">
    <el-card>
      <template #header>银行入金</template>
      <DynamicForm v-if="opUi" :ui="opUi" :model="form" />

      <div style="margin:8px 0; display:flex; gap:8px; align-items:center;">
        <el-select v-model="filters.partnerId" filterable remote :remote-method="searchPartners" reserve-keyword placeholder="选择客户" style="width:260px" @change="onPartnerChange">
      ...
      <div style="margin-top:12px; display:flex; align-items:center; gap:12px;">
        <el-button type="primary" @click="submit" :loading="saving">执行入金</el-button>
...
async function ensureUi(){
  if (!opUi.value){
    const r = await api.get('/schemas/bank_collect')
...
async function submit(){
  ...
  if (allocs.length === 0){ err.value='请选择要分配的未清项并输入金额'; return }
  saving.value = true
  try {
    const body = { header: form.value.header, allocations: allocs }
    const r = await api.post('/operations/bank-collect/allocate', body)
    msg.value = `入金完成：金额 ${r.data?.amount}`
    await loadOpenItems()
  } catch (e:any) {
