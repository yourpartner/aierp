...
const form = reactive<any>({ lines: [], status: 'draft' })
...
onMounted(async () => {
  ...
  uiError.value = ''
  try {
    const r = await api.get('/schemas/sales_order', { params: { lang: lang.value } })
...
watch(
  () => form.lines,
  () => {
    const sum = (Array.isArray(form.lines) ? form.lines : []).reduce((acc: number, it: any) => acc + Number(it?.amount || 0), 0)
    form.amountTotal = sum
  },
  { deep: true }
)
...
async function save() {
  ...
  try {
    ...
    await api.post('/objects/sales_order', body)
