    ...
    <el-card>
      ...
      <el-form ref="formRef" :model="model" :rules="rules" label-width="120px" style="max-width:720px">
        ...
        <el-form-item :label="labels.warehouse" prop="warehouseCode">
          <el-select v-model="model.warehouseCode" :placeholder="labels.warehousePlaceholder" :disabled="isEdit" style="width:250px">
...
const labels = computed(() => ({
  newTitle: lang.value === 'ja' ? '棚番新規' : lang.value === 'en' ? 'New Bin' : '新建棚番',
  editTitle: lang.value === 'ja' ? '棚番編集' : lang.value === 'en' ? 'Edit Bin' : '编辑棚番',
  warehouse: lang.value === 'ja' ? '倉庫' : lang.value === 'en' ? 'Warehouse' : '仓库',
  warehousePlaceholder: lang.value === 'ja' ? '倉庫を選択' : lang.value === 'en' ? 'Select warehouse' : '请选择仓库',
  warehouseRequired: lang.value === 'ja' ? '倉庫を選択してください' : lang.value === 'en' ? 'Please select a warehouse' : '请选择仓库',
  ...
  codeRequired: lang.value === 'ja' ? '棚番コードを入力してください' : lang.value === 'en' ? 'Bin code is required' : '请输入棚番编码',
  codeFormat: lang.value === 'ja' ? '2-4桁の英数字を入力してください' : lang.value === 'en' ? 'Enter 2-4 alphanumeric characters' : '请输入2-4位字母或数字',
  codeDuplicate: lang.value === 'ja' ? 'この棚番コードは既に使用されています' : lang.value === 'en' ? 'This bin code already exists' : '此棚番编码已存在',
  name: lang.value === 'ja' ? '棚番名' : lang.value === 'en' ? 'Bin Name' : '棚番名称',
  namePlaceholder: lang.value === 'ja' ? '棚番名を入力' : lang.value === 'en' ? 'Enter bin name' : '请输入棚番名称',
  nameRequired: lang.value === 'ja' ? '棚番名を入力してください' : lang.value === 'en' ? 'Bin name is required' : '请输入棚番名称',
...
// 过滤掉 code, name, warehouseCode 字段（因为已经单独处理）
  ...
  if (Array.isArray(newUi.form?.layout)) {
    newUi.form.layout = newUi.form.layout.map((block: any) => {
      if (block.type === 'grid' && Array.isArray(block.cols)) {
        block.cols = block.cols.filter((col: any) => !['code', 'name', 'warehouseCode'].includes(col.field))
      }
      return block
    }).filter((block: any) => {
...
const model = reactive<any>({ warehouseCode: '', code: '', name: '', inactive: false })
...
const rules = reactive<FormRules>({
  warehouseCode: [{ required: true, message: labels.value.warehouseRequired, trigger: 'change' }],
  code: [{ required: true, validator: validateCode, trigger: 'blur' }],
  name: [{ required: true, message: labels.value.nameRequired, trigger: 'blur' }]
})

// 检查编码是否重复（同一仓库下）
async function checkCodeDuplicate(warehouseCode: string, code: string): Promise<boolean> {
  try {
    const r = await api.post('/objects/bin/search', {
      page: 1, pageSize: 1,
      where: [
        { field: 'warehouse_code', op: 'eq', value: warehouseCode },
        { field: 'bin_code', op: 'eq', value: code.toUpperCase() }
      ]
    })
...
async function save() {
  ...
  // 检查编码重复
  const isDuplicate = await checkCodeDuplicate(model.warehouseCode, model.code)
  if (isDuplicate) {
  ...
  try {
    ...
    if (isEdit.value) {
      await api.put(`/objects/bin/${editId.value}`, { payload: model })
    } else {
...
async function loadDetail(id: string) {
  try {
    const r = await api.get(`/inventory/bins/${id}`)
...
onMounted(async () => {
  await Promise.all([loadSchema(), loadWarehousesOptions()])
  // 检查是否编辑模式
