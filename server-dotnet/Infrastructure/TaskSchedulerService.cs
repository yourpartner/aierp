...
public sealed class TaskSchedulerService : BackgroundService
{
    ...
    public TaskSchedulerService(NpgsqlDataSource ds, LawDatasetService law, ILogger<TaskSchedulerService>? logger = null)
    {
        _ds = ds;
        _law = law;
        _logger = logger;
...
RETURNING t.id, t.company_code, t.payload, t.next_run_at, t.last_run_at;";
    ...
    private async Task ProcessTaskAsync(SchedulerTaskRecord task, CancellationToken ct)
    {
        ...
        try
        {
            ...
            TaskExecutionOutcome outcome = string.Equals(action, "payroll.batch_execute", StringComparison.OrdinalIgnoreCase)
