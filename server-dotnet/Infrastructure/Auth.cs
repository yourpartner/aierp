...
// 权限辅助：
// - 从请求头或 JWT Claims 解析用户上下文
// - 校验动作权限（RBAC，来源 schema.auth.actions）
// - 构建行级过滤（简化 ABAC，来源 schema.auth.scopes）
public static class Auth
{
    ...
    {
        // 兼容：从 JWT Claims 读取；若没有 JWT，则回落读取自定义请求头
    ...
    public static bool IsActionAllowed(JsonDocument schemaDoc, string action, UserCtx ctx)
    {
        ...
        foreach (var e in entries)
        {
            ...
            else { allowedRoles.Add(e); allowedCaps.Add(e); } // 兼容：不带前缀时，两者都尝试匹配
        }
