...
public class LawDatasetService
{
    ...
    public (decimal rate, string version, string note) GetHealthRate(JsonElement employee, JsonElement policy, DateTime month)
    {
        // 优先策略设置
        string pref = "";
        ...
        var baseAmt = GetNumber(employee, "healthBase"); if (baseAmt<=0) baseAmt = GetNumber(employee, "salaryTotal");
    ...
    public (decimal rate, string version, string note) GetPensionRate(JsonElement employee, JsonElement policy, DateTime month)
    {
        var baseAmt = GetNumber(employee, "pensionBase"); if (baseAmt<=0) baseAmt = GetNumber(employee, "salaryTotal");
        var (ok, rate, ver, note) = QueryFromDb(companyCode: GetString(employee, "companyCode"), kind:"pension", key: null, baseAmt, month);
        if (ok) return (rate, ver!, note!);
        // 二次尝试：忽略区间与公司，取同月最新版本
        try
        {
