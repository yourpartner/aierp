...
// 领域服务：schemas 读写与查询（支持公司级与全局 fallback）
public static class SchemasService
{
    // 读取激活 schema（完整行 to_jsonb(schemas)）。
    // 优先读取 company_code 精确匹配；若不存在，则回落到 company_code IS NULL（全局）。
    public static async Task<JsonDocument?> GetActiveSchema(NpgsqlDataSource ds, string name, string? companyCode)
    {
        ...
        // 2) 全局 fallback（company_code IS NULL）
    ...
    // 保存新版本并激活（公司级或全局）。companyCode 为空表示全局。
