-- Fix admin user for FOS01 on Azure (No ON CONFLICT for roles)
BEGIN;

-- 1. Insert admin user
INSERT INTO users (company_code, employee_code, password_hash, name, user_type, is_active)
VALUES ('FOS01', 'admin', '$2a$11$exnM6jr437kN0R9aDTzjsuJYoL5Dh0gtZM/h4qjDGiYnh3dBQG.Ey', 'FOS Admin', 'internal', true)
ON CONFLICT (company_code, employee_code) DO UPDATE 
SET password_hash = EXCLUDED.password_hash, is_active = true;

-- 2. Link to employee FOS001
UPDATE users
SET employee_id = (SELECT id FROM employees WHERE company_code = 'FOS01' AND employee_code = 'FOS001' LIMIT 1)
WHERE company_code = 'FOS01' AND employee_code = 'admin';

-- 3. Create ADMIN role for FOS01 (Using subquery to avoid conflict error)
INSERT INTO roles (company_code, role_code, role_name, is_active, role_type)
SELECT 'FOS01', 'ADMIN', 'ADMIN', true, 'custom'
WHERE NOT EXISTS (
    SELECT 1 FROM roles WHERE company_code = 'FOS01' AND role_code = 'ADMIN'
);

-- 4. Assign ADMIN role to admin user
INSERT INTO user_roles (user_id, role_id)
SELECT u.id, r.id
FROM users u, roles r
WHERE u.company_code = 'FOS01' AND u.employee_code = 'admin'
  AND r.company_code = 'FOS01' AND r.role_code = 'ADMIN'
  AND NOT EXISTS (
      SELECT 1 FROM user_roles ur2 WHERE ur2.user_id = u.id AND ur2.role_id = r.id
  );

COMMIT;
