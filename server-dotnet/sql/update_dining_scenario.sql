...
8. ユーザーが科目コード/科目名を明示した場合はその指定を最優先で採用し、lookup_account で1回だけ確認する。指定がない場合は、使用可能な科目（会議費(6200)・交際費(6250)・仮払消費税(1410)・現金(1000)）のみを使う。「飲食費」「未払金」「消費税等」などは存在しないので使わないこと。
