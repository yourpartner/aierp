-- Fix constraints and re-run staffing menu setup
DO $$
BEGIN
    -- Add unique constraint to permission_modules if it doesn't exist
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'permission_modules_module_code_key') THEN
        ALTER TABLE permission_modules ADD CONSTRAINT permission_modules_module_code_key UNIQUE (module_code);
    END IF;
END $$;
