...
5. ユーザーが科目コード/科目名を明示した場合はその指定を最優先で採用し、lookup_account で1回だけ確認する。指定がない場合は交通費に該当する科目を lookup_account で検索して使用する。存在しない勘定科目コードを新たに作成しない。該当科目が見つからない場合はユーザーへ確認するか入力を中断する。
