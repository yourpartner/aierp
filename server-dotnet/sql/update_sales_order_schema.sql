-- 更新销售订单 Schema，添加税率和税额字段
-- 在 lines 数组中每个明细行添加 taxRate 和 taxAmount 字段

-- 更新 sales_order schema 定义
UPDATE schemas
SET schema = '{
  "$schema": "https://json-schema.org/draft/2019-09/schema",
  "type": "object",
  "required": ["soNo", "partnerCode", "amountTotal", "orderDate"],
  "properties": {
    "soNo": {
      "type": "string",
      "description": "受注番号"
...
SELECT company_code, name, version, is_active, 
       schema->'properties'->'lines'->'items'->'properties'->'taxRate' as taxRate_schema
FROM schemas 
WHERE name = 'sales_order' AND is_active = true;
